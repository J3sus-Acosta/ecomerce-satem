...
| ADR-018 | 2026-08-24 | Especificación Técnica del Motor B2B | **Fase de Validación y Diseño Completada (`docs/B2B_IMPLEMENTATION_SPEC.md`)** | Especificación técnica finalizada: Hooks en memoria para precios B2C/B2B/Overrides, validación de paso por caja en servidor, formulario de registro comercial con aprobación admin y compatibilidad con WooCommerce Blocks y HPOS. |
| ADR-019 | 2026-08-24 | Capa de Identidad y Registro B2B | **Implementación Completada (Fase 11.2)** | Creados roles B2B (`b2b_toko`, `b2b_restaurant`, `b2b_supermarket`), formulario de solicitud comercial `[satem_b2b_registration_form]`, estado `_satem_b2b_approval_status` (`pending_b2b`, `approved_b2b`, `rejected_b2b`), panel de aprobación en WP Admin y correos en inglés. |
| ADR-020 | 2026-08-24 | Motor de Precios B2B en Memoria | **Implementación Completada (Fase 11.3)** | Módulo `Satem_B2B_Pricing` implementado. Filtrado dinámico en memoria mediante hooks (`woocommerce_product_get_price`, `woocommerce_before_calculate_totals`) sin alterar `_regular_price` en DB. Protección estricta de fuga de precios en REST (`show_in_rest = false`). Fallback automático a precio B2C si el grupo no tiene precio asignado o si el usuario no está aprobado. |
| ADR-021 | 2026-08-24 | Reglas de Caja y Validación de Cantidades B2B | **Implementación Completada (Fase 11.4)** | Módulo `Satem_B2B_Cart` implementado. Restricción de compra por múltiplos de caja máster (`_satem_units_per_box`) exclusiva para usuarios B2B aprobados. Validación en servidor multicapa (`woocommerce_quantity_input_args`, `woocommerce_add_to_cart_validation`, `woocommerce_update_cart_validation`, `woocommerce_check_cart_items`). Comportamiento B2C minorista permanece 100% libre de restricciones. |
| ADR-022 | 2026-09-03 | Arquitectura Frontend Reutilizable por Tema | **Implementación Completada (Fase 12.2)** | Maquetación desacoplada y reutilizable construida en `satem-child` mediante sistema de tokens CSS (`:root`), plantillas semánticas y Hooks de WooCommerce. Prohibidos Page Builders. Rebranding 100% parametrizado sin reescribir código frontend. |
